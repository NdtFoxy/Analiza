...
# Функция для генерации точек фрактала с помощью алгоритма "Игра в хаос" (Chaos Game)
def generate_ifs_points(transformations, iterations=200000, burn_in=100):
    """
    Генерирует точки для фрактала IFS.
    :param transformations: Словарь с аффинными преобразованиями и их вероятностями.
    :param iterations: Общее количество итераций (шагов) алгоритма.
    :param burn_in: Количество "разогревочных" итераций, результаты которых отбрасываются.
    :return: Кортеж из трех NumPy-массивов: (координаты X, координаты Y, индексы цвета).
    """

    # --- Этап 1: Подготовка данных из словаря ---

    # Получаем список ключей (названий) преобразований, например, ['f1', 'f2', 'f3']
    trans_keys = list(transformations.keys())
    # Создаем список вероятностей в том же порядке, что и ключи.
    # Это нужно для взвешенного случайного выбора.
    probabilities = [transformations[k]['prob'] for k in trans_keys]
    
    # Создаем пустые списки для матриц и векторов, чтобы оптимизировать вычисления
    matrices, vectors = [], []
    # Проходим по каждому преобразованию
    for key in trans_keys:
        # Распаковываем 6 коэффициентов аффинного преобразования
        a, b, c, d, e, f = transformations[key]['coeffs']
        # Создаем матрицу 2x2 для линейной части (масштабирование, поворот, наклон)
        matrices.append(np.array([[a, b], [c, d]]))
        # Создаем вектор для части сдвига (перемещения)
        vectors.append(np.array([e, f]))
    
    # --- Этап 2: Проверка и инициализация ---

    # Проверяем, равна ли сумма вероятностей 1.0.
    # Используем isclose с допуском (atol) из-за неточности вычислений с плавающей точкой.
    if not np.isclose(sum(probabilities), 1.0, atol=1e-5):
        # Если сумма не равна 1, выводим ошибку в формате JSON и завершаем программу
        print(json.dumps({
            "error": f"Sum of probabilities is {sum(probabilities)}, should be 1.0"
        }))
        sys.exit(1)
    
    # Инициализируем стартовую точку. Согласно теореме Банаха, можно начать с любой точки. (0,0) - удобно.
    point = np.array([0.0, 0.0])
    # Вычисляем, сколько точек мы на самом деле сохраним (общее число минус "разогревочные")
    output_size = iterations - burn_in
    # Создаем "пустые" (заполненные нулями) массивы NumPy нужного размера для хранения результатов.
    # Это гораздо быстрее, чем добавлять элементы в обычные списки Python в цикле.
    x_coords = np.zeros(output_size)
    y_coords = np.zeros(output_size)
    point_colors = np.zeros(output_size, dtype=int)
    
    # --- Этап 3: Основной цикл генерации ("Игра в хаос") ---
    
    # Запускаем цикл на заданное количество итераций
    for i in range(iterations):
        # Случайным образом выбираем ИНДЕКС одного из преобразований.
        # Выбор взвешенный, т.е. учитываются вероятности из списка `probabilities`.
        choice_index = np.random.choice(len(trans_keys), p=probabilities)
        # Получаем матрицу и вектор, соответствующие выбранному индексу
        matrix = matrices[choice_index]
        vector = vectors[choice_index]
        
        # Вычисляем новую позицию точки: M * p + v (умножение матрицы на вектор + прибавление вектора)
        # Это и есть применение аффинного преобразования.
        point = np.dot(matrix, point) + vector
        
        # Проверяем, закончился ли "разогревочный" период
        if i >= burn_in:
            # Вычисляем индекс для записи в массив результатов
            idx = i - burn_in
            # Записываем координаты новой точки
            x_coords[idx] = point[0]
            y_coords[idx] = point[1]
            # Записываем индекс преобразования, которое создало эту точку (для раскраски)
            point_colors[idx] = choice_index
    
    # --- Этап 4: Возврат результатов ---

    # Возвращаем готовые массивы с координатами и цветами
